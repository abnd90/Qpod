...
Youtube::Youtube(QWidget *parent):QDialog(parent)
{
    setupUi(this);
    pageUrl=new QUrl("http://www.youtube.com/watch?v=aVdO-cx-McA");
    //getDownloadLink(pageUrl);
}

Youtube::Youtube(QUrl* url,QWidget *parent):QDialog(parent),pageUrl(url)
{
    setupUi(this);
    getDownloadLink(pageUrl);
}

QUrl Youtube::getDownloadLink(QUrl* url)
{
    QNetworkAccessManager *manager=new QNetworkAccessManager(this);

    QNetworkRequest request;
    request.setUrl(QUrl(QString("http://www.youtubevdl.110mb.com/?url="+pageUrl->toString())));
...
void Youtube::handleNetworkReply(QNetworkReply* reply)
{
    QByteArray returnedData=reply->readAll();
    std::cerr<<QString(returnedData).toStdString();
